    ...
    if memory_map:
        ...
        if recreate_flag:
            ...
            for i in range(days):
                ...
                if randomize == "total":
                    rand_u = np.random.randint(
                        low=0, high=days_to_sample, size=size)
                    for k in range(size):
                ...
                for j in range(days):
                    filename_j_y = npzfile + \
                        "_{0}_intermediate_y.npy".format(j)
                    filename_j_d = npzfile + \
                        "_{0}_intermediate_d.npy".format(j)
                    filename_j_s = npzfile + \
                        "_{0}_intermediate_s.npy".format(j)
