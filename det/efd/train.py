        ...
        for iteration, batch in enumerate(genval):
            ...
            with torch.no_grad():
                ...
                targets_val = [Variable(torch.from_numpy(ann).type(
                    torch.FloatTensor)).cuda() for ann in targets_val]
                optimizer.zero_grad()
                _, regression, classification, anchors = net(images_val)
                loss, c_loss, r_loss = focal_loss(
                    classification, regression, anchors, targets_val, cuda=True)
                val_loss += loss.item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--freeze", type=int,
                        help="frozen_epoches", default=5)
    parser.add_argument("-u", "--unfreeze", type=int,
                        help="free_epoches", default=5)
    parser.add_argument('-v', "--version", type=int,
                        help='you want to use efficientdet-dX?', default=0)
    parser.add_argument('-b', "--batch_size", type=int,
                        help='just batch_size', default=64)
    parser.add_argument('-p', "--pre_model", type=str,
                        help='just pretrained_model_path default = \'\'', default='')
    parser.add_argument('-s', '--val_split', type=float,
                        help='验证集的比例', default=0.1)
    ...
    pretrained_dict = torch.load(
        args.pre_model) if args.pre_model else load_url(model_urls[phi])
    ...
    Init_Epoch = int(args.pre_model.split(
        '/')[-1].split('-')[0][5:]) if args.pre_model else 0
    ...
    for epoch in range(Init_Epoch, Freeze_Epoch+Unfreeze_Epoch):
        ...
        total_loss, val_loss, weight = fit_one_epoch(
            net, efficient_loss, epoch, epoch_size, epoch_size_val, gen, gen_val, Freeze_Epoch+Unfreeze_Epoch)
        ...
        if min(val_losses) == val_loss:
            ...
            torch.save(weight, './Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' %
