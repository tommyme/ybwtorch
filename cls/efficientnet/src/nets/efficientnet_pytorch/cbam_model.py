...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.bn = bn2d(in_planes)  # 特殊bn
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(x))))
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(x))))
        out = self.bn(avg_out + max_out)  # 特殊bn
        return 2*self.sigmoid(out)  # 特殊bn
...
class cbam_EfficientNet(nn.Module):
    ...
    def from_pretrained(cls, model_name, weights_path=None, advprop=False, num_classes=1000, in_channels=3, cbam=1):
        model = cls.from_name(model_name, override_params={
                              'num_classes': num_classes}, cbam=cbam)
        load_pretrained_weights(
            model, model_name, weights_path, advprop=advprop)
        if in_channels != 3:
