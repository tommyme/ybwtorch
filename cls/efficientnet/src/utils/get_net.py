...
def get_net(net_cfg):
    ...
    if net_cfg['model'][:12] == 'efficientnet': # efficientnet-b4 or efficientnet-cbam-2-b4
        ...
        cbam = int(net_cfg['model'].split('-')[-2])
        return cbam_EfficientNet.from_pretrained('efficientnet-b{}'.format(v),weights_path=net_cfg['pre_model'],num_classes=cfg.num_classes,cbam=cbam)
