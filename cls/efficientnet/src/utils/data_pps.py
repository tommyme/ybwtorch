...
def get_lists(**kwargs):  # mydict指定***.txt的地址, idx < 5
    ...
    cls2id = {list(species.ScientificName)[i]: list(
        species.ID)[i] for i in range(species.shape[0])}
    ...
    anno = pd.read_csv(os.path.join(root, 'annotation.csv')
                       )  # test.csv文件名和真实标签，用于验证
    if opt:
    ...
    if idx == -1:
        ...
        path['train'] = paths2train[:split_n[idx]] + \
            paths2train[split_n[idx+1]:]
        path['val'] = paths2train[split_n[idx]:split_n[idx+1]]
        path['test'] = paths4test

        label['train'] = labels2train[:split_n[idx]] + \
            labels2train[split_n[idx+1]:]
